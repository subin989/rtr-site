...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header top-0 left-0 z-20 flex w-full items-center bg-black ${sticky
                    ...
                    {menuData.map((menuItem, index) => (
                      <li key={menuItem.id} className="group relative">
                        {menuItem.path ? (
                          <Link
                            href={menuItem.path}
                            className={`flex py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:py-6 lg:px-0 hover:border-b-2 hover:pb-0`}
                          >
                            {menuItem.title}
                          </Link>
                        ) : (
